...
def compute_k_search(image_path, k):
    ...
    for dirpath, dirnames, filenames in os.walk(root_folder_image):
        ...
        for filename in filenames:
            ...
            if filename == "cm10x10_features.npy":
                feature_vect_npy=np.load(filename)
            ...
            elif filename =="resnet_layer3_1024_features":
                resnet_layer3_npy = np.load(filename)
                ...
                print(dirpath)
                '''
                resnet_avg_npy=np.load(filename)
                ...
                resnet_fc_npy=np.load(filename)
    ...
    similarity_dict_resnet_fc = sorted(similarity_dict_resnet_fc.items(), key=lambda item: item[1], reverse=True)[:k]
    similarity_dict_resnet_avg = sorted(similarity_dict_resnet_avg.items(), key=lambda item: item[1], reverse=True)[:k]
    similarity_dict_resnet_layer3 = sorted(similarity_dict_resnet_layer3.items(), key=lambda item: item[1], reverse=True)[:k]
    similarity_dict_hog = sorted(similarity_dict_hog.items(), key=lambda item: item[1], reverse=True)[:k]
    similarity_dict_cm = sorted(similarity_dict_cm.items(), key=lambda item: item[1], reverse=True)[:k]
...
def show_k_images(sim_dict, feature, metrica, image_input):
    ...
    plt.show()
    for image in sim_dict:
        print("Nome immagine: " +  str(image))
        print("Feature: " + feature)
        print("Similarity valore: " + str(sim_dict[image]) + "calcolato con metrica " + metrica)
        img = mpimg.imread(str(image)+".jpg")
